...
class GreenScreenImageUploader < CarrierWave::Uploader::Base
  ...
  def removescreen
    manipulate! do |source|
      ...
      original = source
      background = MiniMagick::Image::open(current_path)
      background.clone.fx("p{#{Settings.xcoord},#{Settings.ycoord}}")

      diffmap = original.composite(background, "png") do |c|
        c.compose 'Difference'
        c.alpha 'Off'
      end
      diffmap.modulate "#{Settings.contrast},0"
